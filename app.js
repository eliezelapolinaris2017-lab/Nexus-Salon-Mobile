...
function renderServiceOptions() {
  els.service.innerHTML = ['<option value="" selected>Selecciona servicio</option>']
...
function renderTimeline(items) {
  ...
  const actionButtons = appointment => {
    ...
    const actions = [
      `<button class="btn btn-secondary appt-edit" data-id="${appointment.id}">Editar</button>`,
      `<button class="btn btn-secondary appt-open-client" data-client-id="${esc(appointment.clientId || '')}" data-name="${esc(appointment.clientName || '')}" data-phone="${esc(appointment.clientPhone || '')}">Cliente</button>`,
      `<button class="btn btn-danger appt-delete" data-id="${appointment.id}">Borrar</button>`
    ];

    if (status === 'Pendiente' || status === 'Confirmada') {
      actions.splice(2, 0,
        `<button class="btn btn-ghost appt-status" data-id="${appointment.id}" data-status="Completada">Completar</button>`,
        `<button class="btn btn-ghost appt-status" data-id="${appointment.id}" data-status="No-show">No-show</button>`
      );
    }
  ...
  els.timeline.innerHTML = items.map(i => `
    <article class="timeline-item ${statusClass(i.status)}">
      ...
      <div class="timeline-actions">${actionButtons(i)}</div>
    </article>
  `).join('');
