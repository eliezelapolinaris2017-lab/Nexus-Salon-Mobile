...
const seedAppointments = [
  { id:'s1', clientName:'Miguel Torres', clientPhone:'787-555-1111', service:'Corte + Barba', price:35, date:today, time:'09:00', barber:'Alex', status:'Confirmada', notes:'Cliente VIP' },
  { id:'s2', clientName:'Chris Rivera', clientPhone:'787-555-1112', service:'Fade', price:25, date:today, time:'10:30', barber:'Tony', status:'Pendiente', notes:'' },
  { id:'s3', clientName:'Javier Cruz', clientPhone:'787-555-1113', service:'Corte premium', price:45, date:today, time:'12:00', barber:'Eliezel', status:'Completada', notes:'Pago cash' }
];

const baseServices = [
  { name:'Corte clásico', price:20, time:'30 min' },
  { name:'Fade', price:25, time:'40 min' },
  { name:'Barba', price:15, time:'20 min' },
  { name:'Corte + Barba', price:35, time:'50 min' },
  { name:'Lavado + estilo', price:18, time:'20 min' },
  { name:'Corte premium', price:45, time:'60 min' }
];
...
function enterApp(){
  if (els.pinInput.value.trim() !== DEMO_PIN){
    toast('PIN incorrecto. Aquí no entra nadie por inspiración divina.');
    return;
  }
...
els.appointmentForm.addEventListener('submit', async (e) => {
  ...
  if (firebaseReady && db){
    try{
      ...
      console.error(err);
      appointments.unshift({ id: crypto.randomUUID(), ...payload });
      toast('Firestore falló. Guardé en modo local para que no se frene la operación.');
      render();
    }
  } else {
    appointments.unshift({ id: crypto.randomUUID(), ...payload });
    toast('Modo demo activo. Guardado local.');
    render();
  }
...
async function updateStatus(id, status){
  ...
  if (firebaseReady && db && !String(id).startsWith('s')){
    try{
...
function renderClients(items){
  ...
  items.forEach(item => {
    ...
    row.visits += 1;
    if (item.status !== 'Cancelada' && item.status !== 'No-show') row.spent += Number(item.price || 0);
  });
...
if (firebaseReady && db){
  ...
  onSnapshot(q, (snapshot) => {
    ...
    console.error(error);
    toast('No pude leer Firestore. Modo demo sigue activo.');
  });
}
